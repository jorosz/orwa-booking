...
```
 ...
 naptar.orwa.hu   → orwa-naptar (Caddy, basic_auth):80            │
       ...
       └──────────────────────┘  │   └──────────────────┼──────────┘
                      ...
                      │  SQLite orwa.db (/data)              │
                      │  e-mail → Google Workspace (no auth) │
                      └────────────────────────────────-─────┘
```
...
A service maga **auth-mentes**; a beléptetés a proxykban van (orwa és naptár oldalon).
...
> **csak** `/api/quote` és `/api/book` (+`/healthz`) engedélyezett.

---

## 3. Konfiguráció

Az alapszolgáltatásnak **nincs titka** (az e-mail auth nélkül megy, lásd §5);
egyetlen titok a napi backuphoz kell — az AWS-kulcs (lásd [BACKUP.md](BACKUP.md)).
...
| `BACKUP_BUCKET` | S3 bucket a napi mentéshez (ha nincs, a backup kimarad) | nincs |
| `AWS_REGION` | a backup-bucket régiója | `eu-central-1` |
| `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` | a backup IAM-user kulcsa (`env_file`) | nincs |
...
- **Csak elérhető ajánlat jöhet:** ami nem szabad a naptárban, az **meg sem jelenik** a válaszban
  (válaszban nincs `available` flag csak a logban). Vendégház csak ha `guestHouseFree`; apartman
...
- **Egyetlen `/data` volume:** a `orwa.db` (+WAL). 
...
- **Mentés & adattörlés:** napi S3-backup, a törölt foglalások végleges törlése a
  `departure` után, és a `quote_requests` PII anonimizálása 3 hónap után. A séma ezt
  támogatja (`secure_delete`, hard-purge). Részletek: [BACKUP.md](BACKUP.md).
